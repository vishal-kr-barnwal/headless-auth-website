...
## Why name **VedaVerify**?
- **Veda** derives from Hindu Mythology, which consists of four Vedas known as a source of truth for all rituals and,
culture. These are also symbolized in the Logo in form of four dots.
- **Verify** is depiction of what platform do i.e., verify user & grant access (Required & Controlled) to a Certain
Application.
- **L** Bars around dots in Logo present each as isolated entity.
- **Inner Circle** is what connects all these Plugins or Knowledge sources together with a common interface.
